...
from pipeline_cascade0123 import Zero1to3StableDiffusionPipeline, CCProjection
...
def main(args):
    ...
    feature_extractor = CLIPFeatureExtractor.from_pretrained(args.pretrained_model_name_or_path, subfolder="feature_extractor", revision=args.revision)
    ...
    #init cc_projection
    cc_projection = CCProjection.from_pretrained(args.pretrained_model_name_or_path, subfolder="cc_projection", revision=args.revision)
    cc_projection.requires_grad_(True)
    cc_projection.train()

    refine_pipe = Zero1to3StableDiffusionPipeline.from_pretrained(args.pretrained_model_name_or_path, torch_dtype=torch.float16)
    ...
    if args.use_ema:
        ...
        ema_cc_proj = EMAModel(cc_projection.parameters(), model_cls=CCProjection, model_config=cc_projection.config)
    ...
    rf_image_transforms = torchvision.transforms.Compose(
        [
            torchvision.transforms.Resize((args.resolution, args.resolution)),  # 256, 256
    ...
    if args.use_ema:
        unet, cc_projection, optimizer, train_dataloader, validation_dataloader, train_log_dataloader, lr_scheduler, ema_unet, ema_cc_proj = accelerator.prepare(
            unet, cc_projection, optimizer, train_dataloader, validation_dataloader, train_log_dataloader, lr_scheduler, ema_unet, ema_cc_proj
        )
    else:
    ...
    if args.use_ema:
        ...
        ema_cc_proj.to(accelerator.device)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet, cc_projection):
                ...
                input_image = batch["image_cond"].to(dtype=weight_dtype)
                pose = batch["T"].to(dtype=weight_dtype)
                pose2 = batch["T2"].to(dtype=weight_dtype)
                pose3 = batch["T3"].to(dtype=weight_dtype)
                pose4 = batch["T4"].to(dtype=weight_dtype)

                target_pose = batch["target_RT"].to(dtype=weight_dtype)
                cond_pose = batch["cond_RT"].to(dtype=weight_dtype)
                cond_pose2 = batch["cond_RT2"].to(dtype=weight_dtype)
                cond_pose3 = batch["cond_RT3"].to(dtype=weight_dtype)
                cond_pose4 = batch["cond_RT4"].to(dtype=weight_dtype)

                tgt_T_mul = torch.cat(((target_pose - cond_pose),
                                        (cond_pose2 - cond_pose),
                                        (cond_pose3 - cond_pose),
                                        (cond_pose4 - cond_pose) ),
                                        dim=0)

                tgt_T_mul[:,0] = (tgt_T_mul[:,0] / (2 * math.pi) * 360)
                tgt_T_mul[:,1] = ((tgt_T_mul[:,1] % (2 * math.pi)) / (2 * math.pi) * 360)

                B,C,H,W = input_image.shape

                cond_image = target_pipe(input_imgs=input_image.repeat(4,1,1,1),
                                            prompt_imgs=input_image.repeat(4,1,1,1),
                                            poses=tgt_T_mul.tolist(),
                                            height=H,
                                            width=W,
                                            guidance_scale=3.0,
                ...
                cond_image = torch.from_numpy(np.transpose(cond_image_coarse, (0,3,1,2)))
                cond_image = rf_image_transforms(cond_image)

                tgt_img_coarse = cond_image[:B]
                cond_image2 = cond_image[B:2*B]
                cond_image3 = cond_image[2*B:3*B]
                cond_image4 = cond_image[3*B:4*B]
                ...
                if do_classifier_free_guidance:  #support classifier-free guidance, randomly drop out 5%
                    ...
                    tgt_prompt_embeds = _encode_image(image_encoder, tgt_img_coarse, gt_latents.device, gt_latents.dtype, False)
                    tgtpose_prompt_embeds = torch.zeros_like(pose_prompt_embeds)
                    cond2_prompt_embeds = _encode_image(image_encoder, cond_image2, gt_latents.device, gt_latents.dtype, False)
                    cond2pose_prompt_embeds = _encode_pose(pose2, gt_latents.device, gt_latents.dtype, False)
                    cond3_prompt_embeds = _encode_image(image_encoder, cond_image3, gt_latents.device, gt_latents.dtype, False)
                    cond3pose_prompt_embeds = _encode_pose(pose3, gt_latents.device, gt_latents.dtype, False)
                    cond4_prompt_embeds = _encode_image(image_encoder, cond_image4, gt_latents.device, gt_latents.dtype, False)
                    cond4pose_prompt_embeds = _encode_pose(pose4, gt_latents.device, gt_latents.dtype, False)
                    ...
                    tgt_prompt_embeds = torch.where(prompt_mask, null_conditioning, tgt_prompt_embeds)

                    prompt_embeds = torch.cat([img_prompt_embeds, pose_prompt_embeds], dim=-1)
                    prompt_embeds = cc_projection(prompt_embeds)
                    
                    tgt_prompt_embeds = torch.cat([tgt_prompt_embeds, tgtpose_prompt_embeds], dim=-1)
                    tgt_prompt_embeds = cc_projection(tgt_prompt_embeds)
                    cond2_prompt_embeds = torch.cat(cond2_prompt_embeds, cond2pose_prompt_embeds)
                    cond2_prompt_embeds = cc_projection(cond2_prompt_embeds)
                    cond3_prompt_embeds = torch.cat(cond3_prompt_embeds, cond3pose_prompt_embeds)
                    cond3_prompt_embeds = cc_projection(cond3_prompt_embeds)
                    cond4_prompt_embeds = torch.cat(cond4_prompt_embeds, cond4pose_prompt_embeds)
                    cond4_prompt_embeds = cc_projection(cond4_prompt_embeds)
                    ...
                    tgt_prompt_embeds = _encode_image_with_pose(image_encoder, cc_projection, tgt_img_coarse, torch.zeros_like(pose), gt_latents.device, weight_dtype, False)
                    prompt_embeds = torch.cat([prompt_embeds, tgt_prompt_embeds], dim=1)
            ...
            if accelerator.sync_gradients:
                if args.use_ema:
                    ema_unet.copy_to(refine_pipe.unet.parameters())
                    ema_cc_proj.copy_to(refine_pipe.cc_projection.parameters())


                    ema_unet.step(unet.parameters())
                    ema_cc_proj.step(cc_projection.parameters())
                ...
                if accelerator.is_main_process:
                    if global_step % args.checkpointing_steps == 0:
                        ...
                        if args.use_ema:
                            #Store the UNet parameters temporarily
                            ema_unet.store(unet.parameters())
                            ema_unet.copy_to(unet.parameters())
                            ema_cc_proj.store(cc_projection.parameters())
                            ema_cc_proj.copy_to(cc_projection.parameters())
